...
description: Thorough, user-invoked tech debt and architecture audit of the current codebase. Produces a dated report under docs/audit-reports/ with file-cited findings, severity, effort estimates, and a required "looks bad but is actually fine" section. Use when the user asks for a debt audit, codebase health check, architecture review, or code quality assessment of an entire repo. Does not auto-invoke.
disable-model-invocation: true
---

# Tech Debt Audit

A Claude Code skill that conducts a deliberate, opinionated audit of an entire codebase and produces a dated report at `docs/audit-reports/tech-debt-audit-<YYYY-MM-DD>.md` with cited findings.
...
Write to `docs/audit-reports/tech-debt-audit-<YYYY-MM-DD>.md` (today's date; create the `docs/audit-reports/` dir if missing) with this structure:
...
If prior reports exist in `docs/audit-reports/`, read the most recent one (latest date in filename) first. Mark resolved findings as `RESOLVED`, update stale ones, and tag new findings with `NEW`. Write the result to a **new** dated file — never overwrite a prior report. This turns the audit into a living document tracked over time.
...
That's it. Output goes to a dated file under `docs/audit-reports/`. First run takes 5–20 minutes depending on repo size; subsequent runs in repeat-run mode are faster.
